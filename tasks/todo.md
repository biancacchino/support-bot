...
### Correction, found in Phase 11: the container suite was broken

The line above was true only when the *whole repo* was mounted into the container.
`pytest.ini` was never copied into the image, so the command the README actually documents ran with
`asyncio_mode=strict`, handed every async fixture to its test as an un-awaited coroutine, and failed 85 of
161 tests.

CI never caught it because CI runs on the host, off a plain checkout, where `pytest.ini` is simply there.
So the image and CI disagreed about whether the tests passed, and the image was the one telling the truth.

Fixed by copying `pytest.ini` into the image rather than by mounting more of the repo - mounting the repo
is what drags `.env` in, and a local `.env` deciding what the tests conclude is the Phase 4 bug all over
again.
...
- [x] 11.1 320 Bitext queries sampled and frozen (`eval/queries.jsonl`), mapped to KB ground truth
- [x] 11.2 Recall@k, Precision@k and MRR over retrieval and rerank
- [x] 11.3 Out-of-domain queries (Bitext insurance) to measure the escalation false-negative rate
- [x] 11.4 Benchmark report: `docs/benchmark.md`, generated by `scripts/eval.py`

Acceptance: met and verified on 2026-07-12. 160 tests pass, 1 xfail, ruff clean.
The eval never calls Gemini: what is retrieved, how it ranks, and whether the gate escalates are all
decided before the LLM is reached, so the benchmark costs nothing to re-run.

Ground truth is the `intents:` frontmatter already in `kb/`, not a hand-written mapping.
A query labelled `track_order` is correct if it retrieves a document claiming `track_order`, and
`scripts/check_kb_coverage.py` already fails the build if that frontmatter is wrong.

The sample is committed rather than drawn fresh each run.
A benchmark that resamples on every run measures a different thing every run, and two numbers from two
samples are not comparable.

### The headline: neither PRD target is met, and tuning cannot fix it

| | Result | Target |
|---|---|---|
| Deflection | 25.0% | >= 40% |
| False answers | 1.9% | < 2% |
| Negatives leaked | 0 of 100 | - |

No threshold in the sweep hits both.
0.05 reaches 43.2% deflection but breaks the false-answer promise 2.5x over; 0.5 honours the promise and
gives up deflection to do it. Bianca chose 0.5: the false-answer cap is a promise to a customer and
deflection is a promise to a budget, and when they collide the customer wins.

That reverses Phase 3, which measured 0.35 on 17 hand-written queries and rejected 0.5 as too strict.
Phase 3 was not wrong about its 17 queries. It was wrong to believe 17 queries.
Real Bitext phrasing ("correct order", "i cant afford order", "could i modify oredr") scores far lower on
the cross-encoder than anything written by someone who already knew what the KB said, and the 8 clean smoke
queries could not have shown that.

The cost of 0.5 is paid in the open, in `KNOWN_LOST_DEFLECTIONS` in `tests/test_reranker.py`: one of the 8
smoke queries now escalates at 0.455. It is an explicit set rather than a relaxed assertion, so a ninth
query starting to escalate fails the build instead of passing quietly.

### What the numbers actually say is wrong

**Retrieval is not the bottleneck; ranking and calibration are.**
Stage 1 recall@10 is 96.4% - the right document is nearly always retrieved. After reranking, recall@4 is
86.4% but top-1 accuracy is only 55.9%.

**The cross-encoder is a good ranker and a bad confidence signal.**
This is the finding worth saying out loud. `payment_issue` has 100% recall@4 and an MRR of 0.95 - the
reranker puts the right document first almost every time - and deflects *zero* of its 10 queries, because
every one scores below the gate. `delivery_options` is the same story. The score the gate reads is not
calibrated to whether the ranking is right, so the gate escalates questions the reranker had already
answered correctly.

Sigmoid-squashing an uncalibrated logit does not make it a probability. `CONFIDENCE_THRESHOLD` is being
read as "how likely is this answer correct" and it is not that number.

**The KB has real gaps, and the by-intent table names them.**
`delivery_options` (50% recall@4), `delivery_period` (40%) and `check_refund_policy` (50%) are the worst
three, and they are missing coverage rather than mis-ranked: "how long does delivery take" lands in
`placing-an-order`, and "in which cases can I ask for a refund" lands in `requesting-a-refund` instead of
`refund-policy`.

### Where the deflection comes back from
